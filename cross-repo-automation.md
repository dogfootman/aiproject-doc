...
| 라벨명 | 색상 | 설명 | 다음 상태 |
|--------|------|------|----------|
| `spec-pending` | 🟡 #fbca04 | Spec 대기 | ready-to-implement |
| `ready-to-implement` | 🟣 #7057ff | 구현 가능 | in-progress |
| `in-progress` | 🟠 #f9d0c4 | 구현중 | dev-complete |
| `dev-complete` | 🔵 #1d76db | 구현 완료 | test-complete |
| `test-complete` | 🟢 #0e8a16 | 테스트 완료 | CLOSED |

#### Doc Repo 라벨

| 라벨명 | 색상 | 설명 | Source 상태 |
|--------|------|------|-------------|
| `spec-pending` | 🟡 #fbca04 | Spec 대기 | spec-pending |
| `spec-writing` | 🟠 #f9d0c4 | Spec 작성중 | spec-pending |
| `spec-complete` | 🟣 #7057ff | Spec 완료 | ready-to-implement |
| `impl-started` | 🔵 #1d76db | 구현 시작됨 | in-progress |
| `impl-complete` | 🟢 #0e8a16 | 구현 완료 | dev-complete |
| `verified` | ✅ #0e8a16 | 검증 완료 | test-complete |

### 8.3 자동화 워크플로우

| 자동화 | 트리거 | Source 변경 | Doc 변경 | 위치 |
|--------|--------|-------------|----------|------|
| **A** | Doc `spec-complete` 라벨 | → `ready-to-implement` | - | Doc Repo |
| **B** | Source `in-progress` 라벨 | - | → `impl-started` | Source Repo |
| **C** | Source PR 머지 | → `dev-complete` | → `impl-complete` | Source Repo |
| **D** | Source `test-complete` 라벨 | - | → `verified` | Source Repo |
...
Doc Repo:
.github/workflows/
└── notify-source-repo.yml       # 자동화 A

Source Repo:
.github/workflows/
└── sync-to-doc-repo.yml         # 자동화 B, C, D
```
...
| Repo | Secret 이름 | 용도 |
|------|-------------|------|
| Doc Repo | `SOURCE_REPO_TOKEN` | Source Repo 이슈 업데이트 |
| Source Repo | `DOC_REPO_TOKEN` | Doc Repo 이슈 업데이트 |
...
```bash
# 진행 상태 라벨
gh label create "spec-pending" --description "Spec 작성 대기중" --color "fbca04" --repo <owner>/<source-repo>
gh label create "ready-to-implement" --description "구현 가능" --color "7057ff" --repo <owner>/<source-repo>
gh label create "in-progress" --description "구현 진행중" --color "f9d0c4" --repo <owner>/<source-repo>
gh label create "dev-complete" --description "구현 완료, 테스트 대기" --color "1d76db" --repo <owner>/<source-repo>
gh label create "test-complete" --description "테스트 완료" --color "0e8a16" --repo <owner>/<source-repo>
```
...
```bash
# 진행 상태 라벨
gh label create "spec-pending" --description "Spec 대기" --color "fbca04" --repo <owner>/<doc-repo>
gh label create "spec-writing" --description "Spec 작성중" --color "f9d0c4" --repo <owner>/<doc-repo>
gh label create "spec-complete" --description "Spec 완료" --color "7057ff" --repo <owner>/<doc-repo>
gh label create "impl-started" --description "구현 시작됨" --color "1d76db" --repo <owner>/<doc-repo>
gh label create "impl-complete" --description "구현 완료" --color "0e8a16" --repo <owner>/<doc-repo>
gh label create "verified" --description "검증 완료" --color "0e8a16" --repo <owner>/<doc-repo>
```
